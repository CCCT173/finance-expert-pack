...
def get_company_news(ticker, days=7, market_type=None):
    """获取公司新闻。

    - A股(market_type='a')：走 akshare（东方财富个股新闻 / 财联社电报）。
      **注意**：yfinance 不提供 A 股新闻源，原实现会让 A 股标的永远返回"未找到新闻"。
    - 港股 / 美股 / 其他：沿用 yfinance。
    """
    # ---- A 股分支 ----
    if market_type == "a":
        news = []
        try:
            ...
            try:
                df = ak.stock_news_em(symbol=ticker)  # ticker 形如 "600066"
            ...
            # 财联社电报（近期快讯，作为补充舆情来源）
            try:
                cls = ak.stock_telegraph_cls()
                if cls is not None and len(cls) > 0:
                    for _, row in cls.head(50).iterrows():
                        title = str(row.get("标题", "") or row.get("新闻标题", "")).strip()
                        if title:
                            news.append({
                                "title": title,
                                "publisher": "财联社",
...
def main():
    ...
    # 获取新闻（A股走 akshare 分支，其余走 yfinance）
    news_list, company_info = get_company_news(ticker_norm, days, market_type=market_type)
